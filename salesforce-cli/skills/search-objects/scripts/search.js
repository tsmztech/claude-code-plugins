#!/usr/bin/env node
/**
 * search.js - Search for Salesforce standard and custom objects by name pattern
 *
 * Queries the EntityDefinition sObject via SOQL to find objects matching
 * a search pattern by API name or label, with server-side filtering.
 *
 * Usage:
 *   node search.js <pattern>                              # e.g. "Account"
 *   node search.js <pattern> --target-org <alias>         # Specific org
 *   node search.js <pattern> --custom-only                # Only custom objects
 *   node search.js <pattern> --standard-only              # Only standard objects
 *   node search.js <pattern> --queryable                  # Only queryable objects
 *   node search.js <pattern> --raw                        # Raw JSON output
 *
 * Options:
 ...
 *
 * Examples:
 *   node search.js Account              # Finds Account, AccountHistory, etc.
 *   node search.js "Order Work"         # Finds WorkOrder, WorkOrderLineItem, etc.
...
for (let i = 0; i < args.length; i++) {
  switch (args[i]) {
    ...
    case "-h":
      console.log(
        [
          "Usage: node search.js <pattern> [options]",
          "",
          ...
          "",
          "Examples:",
          "  node search.js Account",
          '  node search.js "Order Work"',
          "  node search.js Order --custom-only",
          "  node search.js Case --target-org myDevOrg",
        ].join("\n")
      );
      process.exit(0);
      break;
...
if (!searchPattern) {
  console.error(
    'ERROR: Search pattern is required. Usage: node search.js <pattern>\n' +
    'Example: node search.js Account'
  );
  process.exit(1);
}
...
// Each term must match QualifiedApiName (EntityDefinition does not support OR/disjunctions)
...
if (queryableOnly) matchingObjects = matchingObjects.filter((o) => o.queryable);
