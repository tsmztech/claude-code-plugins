#!/usr/bin/env node
/**
 * describe-object.js - Describe a Salesforce object's schema metadata
 *
 * Shared utility for the salesforce-dev plugin. Retrieves detailed field-level
 * metadata for any Salesforce object including standard and custom fields,
 * relationships, picklist values, and field properties.
 *
 * Usage:
 *   node describe-object.js <ObjectName>                              # e.g. Account
 *   node describe-object.js <ObjectName> --target-org <alias>         # Specific org
 *   node describe-object.js <ObjectName> --fields-only                # Just field list
 *   node describe-object.js <ObjectName> --field <FieldName>          # Single field detail
 *   node describe-object.js <ObjectName> --relationships              # Only relationships
 *   node describe-object.js <ObjectName> --json                       # Raw JSON output
 *
 * Options:
...
for (let i = 0; i < args.length; i++) {
  switch (args[i]) {
    ...
    case "-h":
      console.log(
        [
          "Usage: node describe-object.js <ObjectName> [options]",
          "",
          "Describe a Salesforce object's schema metadata.",
          "",
          "Options:",
...
if (!objectName) {
  console.error(
    "ERROR: Object name is required. Usage: node describe-object.js <ObjectName>"
  );
  process.exit(1);
}
...
function printObjectSummary(result, fields) {
  console.log(
    `=== ${result.name || "Unknown"} (${result.label || ""}) ===`
  );
...
function printRelationships(result, fields) {
  ...
  console.log(
    `=== Relationships on ${result.name || "Unknown"} (${relFields.length} fields) ===`
  );
  console.log();
  console.log(
    `${pad("Field", 35)} ${pad("Relationship", 30)} ${pad("References", 30)} Type`
  );
  ...
  if (childRels.length) {
    ...
    console.log();
    console.log(
      `${pad("Child Object", 35)} ${pad("Relationship Name", 35)} Field`
    );
    ...
    childRels
      .slice()
      .sort((a, b) =>
        (a.childSObject || "").localeCompare(b.childSObject || "")
      )
      .forEach((cr) => {
        console.log(
...
function printFieldsTable(fields) {
  ...
  console.log();
  console.log(
    `${pad("Name", 40)} ${pad("Label", 30)} ${pad("Type", 15)} ${pad("Length", 8)} ${pad("Nillable", 10)} Ref`
  );
